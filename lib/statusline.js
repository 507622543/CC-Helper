...
const YOLO_BACKUP = _path.join(process.env.HOME || process.env.USERPROFILE || '', '.claude', '.yolo-backup.json');
function isYoloActive() {
    try { return _fs.existsSync(YOLO_BACKUP); } catch { return false; }
}
...
function buildStatusLine(data) {
    ...
    const segments = [
        // YOLO indicator (if active)
        ...(isYoloActive() ? [`${bold}${fg(203)}YOLO${RESET}`] : []),
        // Model
